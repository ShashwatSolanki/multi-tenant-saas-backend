...
def test_project_task_priority_and_comment_flow(client):
    headers = auth_headers(client, "owner@acme.test", "password123")
    project = client.post("/api/v1/projects", headers=headers, json={
        "name": "Aegis MVP",
        "description": "Secure multi-tenant project",
    })
    assert project.status_code == 201
    assert project.json()["description"] == "Secure multi-tenant project"
    project_id = project.json()["project_id"]

    task = client.post(f"/api/v1/projects/{project_id}/tasks", headers=headers, json={
        "title": "Ship API",
