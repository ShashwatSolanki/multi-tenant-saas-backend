...
def test_registration_login_and_health():
...
def test_project_and_task_flow():
    headers = auth_headers("owner@acme.test", "password123")
...
def test_cross_tenant_project_isolation():
    ...
    assert response.status_code == 201
    headers = auth_headers("owner@other.test", "password123")
