...
from app.models.models import Comment, Project, ProjectMember, Task, TaskMember, User
from app.schemas import CommentResponse, ProjectMemberResponse, ProjectResponse, TaskResponse, TaskUpdate, UserResponse
...
def scoped_users(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    if user.role.name != "Member":
        users = db.scalars(
            select(User).where(User.tenant_id == user.tenant_id, User.deleted_at.is_(None)).order_by(User.created_at)
        ).all()
    else:
        users = db.scalars(
            select(User)
...
def scoped_project_members(project_id: UUID, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    rows = db.execute(
        ...
        .where(ProjectMember.project_id == project_id, User.tenant_id == user.tenant_id)
    ).all()
    return [
...
def scoped_tasks(project_id: UUID, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    tasks = db.scalars(
        select(Task)
        .join(Project)
        .where(Task.project_id == project_id, Project.tenant_id == user.tenant_id)
    ...
    if user.role.name == "Member":
        tasks = [
            ...
            or any(member.user_id == user.user_id for member in task.collaborators)
        ]
...
def scoped_task_update(task_id: UUID, data: TaskUpdate, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    task = db.scalar(
        select(Task).join(Project).where(Task.task_id == task_id, Project.tenant_id == user.tenant_id)
    )
    if not task:
    ...
    is_participant = (
        ...
        or task.created_by == user.user_id
        or db.scalar(select(TaskMember).where(TaskMember.task_id == task_id, TaskMember.user_id == user.user_id)) is not None
    )
    if user.role.name == "Member":
    ...
    if "assignee_id" in payload:
        assignee = db.scalar(
            select(User).where(
                User.user_id == payload["assignee_id"],
                User.tenant_id == user.tenant_id,
                User.deleted_at.is_(None),
        ...
        if not assignee:
            raise HTTPException(400, "Assignee must be an active user in the tenant")
        if not is_member(db, task.project_id, assignee.user_id):
...
def scoped_audit_logs(user: User = Depends(get_current_user)):
    if user.role.name == "Member":
